...
async function processEntrypoints(
  ...
  let htmlEntrypoints: null | ScannedHtmlEntrypoint[];
  let bundleEntrypoints: string[];
  // If entrypoints are JS:
  if (entrypointFiles.every((f) => f.endsWith('.js'))) {
    htmlEntrypoints = null;
    bundleEntrypoints = entrypointFiles;
  ...
  if (entrypointFiles.every((f) => f.endsWith('.html'))) {
    ...
    htmlEntrypoints = rawHtmlEntrypoints.filter(isTruthy);
    if (
...
async function bundleAnalyzeBuildDirectory(
...
export async function runBuiltInOptimize(config: SnowpackConfig) {
  ...
  const userEntrypoints = await resolveEntrypoints(options.entrypoints, allBuildFiles);
  ...
  const {manifest, outputFiles} = await bundleAnalyzeBuildDirectory(
    bundleEntrypoints,
    config,
    esbuildService,
  );
